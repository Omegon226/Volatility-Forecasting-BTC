...
def make_forecast():
    df_now, df_forecast, df_forecast_norm, df_btc_usdt = get_data_for_forecast_page(model="dlinear")
    ...
    """)
    st.plotly_chart(return_hist_plot(df_forecast, model="dlinear"), use_container_width=True)
...
def main():
    ...
    st.markdown("# DLinear Forecast")

    st.markdown(r"""
        На этой странице представлены прогнозы, рассчитанные с помощью модели **DLinear**.
        DLinear — это модель машинного обучения, разработанная для прогнозирования временных рядов. 
        Модель декомпозирует временной ряд на трендовые и сезонные компоненты, после чего применяет 
        линейные преобразования для каждой из них. Такой подход позволяет эффективно захватывать как 
        долгосрочные тенденции, так и регулярные колебания в данных, обеспечивая точные и стабильные прогнозы. 
        Благодаря своей простоте и вычислительной эффективности, DLinear сочетает преимущества традиционных 
        линейных моделей с возможностями современной обработки временных рядов.
    """)
    ...
    if st.button("Перезапустить расчёты", key="DLinear_forecast", type="primary"):
        st.rerun()

...
def make_forecast():
    df_now, df_forecast, df_forecast_norm, df_btc_usdt = get_data_for_forecast_page(model="nlinear")
    ...
    """)
    st.plotly_chart(return_hist_plot(df_forecast, model="nlinear"), use_container_width=True)
...
def main():
    ...
    st.markdown("# NLinear Forecast")

    st.markdown(r"""
        На этой странице представлены прогнозы, рассчитанные с помощью нейронной сети **NLinear**.
        NLinear — это архитектура нейронной сети, разработанная для прогнозирования временных рядов. 
        Модель использует линейные преобразования и специализированные слои для эффективного выявления и 
        моделирования временных зависимостей в данных. Благодаря своей простоте и высокой вычислительной 
        эффективности, NLinear обеспечивает точные прогнозы, сочетая преимущества традиционных линейных
        моделей с гибкостью современных нейронных сетей.
    """)
    ...
    if st.button("Перезапустить расчёты", key="NLinear_forecast", type="primary"):
        st.rerun()
